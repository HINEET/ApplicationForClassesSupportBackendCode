<?php
    ...
    $result = mysqli_query($con, "SELECT COURSE.courseID, COURSE.courseGrade, COURSE.courseTitle, COURSE.courseProfessor, COURSE.courseCredit
        , COURSE.courseDivide, COURSE.coursePersonnel, COURSE.courseTime FROM SCHEDULE, COURSE, USER WHERE USER.userMajor IN (SELECT userMajor FROM 
        USER WHERE userID = '$userID') AND USER.userID = SCHEDULE.userID AND SCHEDULE.courseID = COURSE.courseID GROUP BY 
